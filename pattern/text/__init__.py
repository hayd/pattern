...
def find_keywords(string, parser, top=10, frequency={}, ignore=("rt",), pos=("NN",), **kwargs):
    ...
    for i, chunk in enumerate(t):
        ...
        for w in chunk:
            ...
            m[k][0] += old_div(1, float(n))
            m[k][1] |= 1 if len(ctx[w[0]]) > 1 else 0
            m[k][2] |= 1 if old_div(i, float(len(t))) <= 0.25 else 0
    ...
    if frequency:
        for k in m:
            if not k.isalpha(): # @username, odd!ti's
                ...
                df = old_div(1.0, max(frequency.get(w[0].lower(), frequency.get(k, 0)), 0.0001))
                df = log(df)
            m[k][0] *= df
            #print k, m[k]
    # Sort candidates alphabetically by total score.
    # The harmonic mean will emphasize tf-idf score.
    hmean = lambda a: old_div(len(a), sum(old_div(1.0, (x or 0.0001)) for x in a))
...
def avg(list):
    return old_div(sum(list), float(len(list) or 1))
...
class Spelling(lazydict):
    ...
    def suggest(self, w):
        ...
        candidates = sorted(((old_div(p, s), w) for p, w in candidates), reverse=True)
...
def language(s):
    ...
    for xx in LANGUAGES:
        ...
        p[xx] = old_div(sum(1 for w in s if w in lexicon), n)
