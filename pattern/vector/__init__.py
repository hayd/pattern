...
class Document(object):
    ...
    def term_frequency(self, word):
        ...
        """
        return old_div(float(self.terms.get(word, 0)), (self.count or 1))
    ...
    def keywords(self, top=10, normalized=True):
        ...
        v = ((old_div(f,n), w) for w, f in list(self.vector.items()))
...
def cosine_similarity(v1, v2):
    ...
    s = old_div(float(s), (l2_norm(v1) * l2_norm(v2) or 1))
    return s
...
def tf_idf(vectors=[], base=2.71828): # Euler's number
    ...
    for v in vectors: 
        for f in v: # Modified in-place.
            s(v, f, v[f] * (log(old_div(n, df[f]), base)))
    return vectors
...
def distance(v1, v2, method=COSINE):
    ...
    if method == HAMMING:
        ...
        d = old_div(d, float(max(len(v1), len(v2)) or 1))
        return d
...
def entropy(p=[], base=None):
    ...
    return -sum(x / s * log(old_div(x, s), b) for x in p if x != 0) or 0.0
...
class Model(object):
    ...
    def keywords(self, top=10, normalized=True):
        ...
        v = ((old_div(f,n), w) for w, f in list(self._df.items()))
        v = heapq.nsmallest(top, v, key=lambda v: (-v[0], v[1]))
        return v
    ...
    def inverse_document_frequency(self, word, base=2.71828):
        ...
        if df == 1.0: 
            return 0.0
        return log(old_div(1.0, df), base)
    ...
    def density(self):
        ...
        """
        return old_div(float(sum(len(d.vector) for d in self.documents)), len(self.vector) ** 2)
...
#   def cluster(self, method=HIERARCHICAL, k=1, iterations=1000)
    ...
    def posterior_probability(self, word, type):
        ...
        if not self._pp:
            ...
            for d in self.documents:
                for f in d.terms:
                    p2[f][d.type] += old_div(1, p1[d.type])
                    p3[f] += 1
            for t in p1:
                for f in p3:
...
class Apriori(object):
    ...
    def Lk(self, sets, candidates, support=0.0):
        ...
        """
        Lk, x = {}, old_div(1.0, (len(sets) or 1)) # relative count
...
def mean(iterable, length=None):
    ...
    if length is None:
        ...
        length = len(iterable)
    return old_div(sum(iterable), float(length or 1))
...
class Classifier(object):
    ...
    def weighted_random_baseline(self):
        ...
        return sum([(old_div(x, n)) ** 2 for x in list(self.distribution.values())])
    ...
    def skewness(self):
        ...
        """
        def moment(a, m, k=1):
            return old_div(sum([(x-m)**k for x in a]), (len(a) or 1))
        ...
        m = old_div(float(sum(a)), len(a)) # mean
        return old_div(moment(a, m, 3), (moment(a, m, 2) ** 1.5 or 1))
...
class ConfusionMatrix(defaultdict):
    ...
    def test(self, target=None):
        ...
        for type, TP, TN, FP, FN in self.split():
            if type == target or target is None:
                ...
                A.append(old_div(float(TP + TN), ((TP + TN + FP + FN))))
                P.append(old_div(float(TP), ((TP + FP) or 1)))
                R.append(old_div(float(TP), ((TP + FN) or 1)))
        # Macro-averaged:
        A = old_div(sum(A), (len(A) or 1.0))
        P = old_div(sum(P), (len(P) or 1.0))
        R = old_div(sum(R), (len(R) or 1.0))
    ...
    def auc(self, k=10):
        ...
        roc = [(0.0, 0.0), (1.0, 1.0)]
        for type, TP, TN, FP, FN in self.split():
            x = FPR = old_div(float(FP), ((FP + TN) or 1)) # false positive rate
            y = TPR = old_div(float(TP), ((TP + FN) or 1)) #  true positive rate
            roc.append((x, y))
...
def K_fold_cross_validation(Classifier, documents=[], folds=10, **kwargs):
    ...
    K = len(folds)
    u = old_div(float(sum(f)), (K or 1.0))
    o = old_div(float(sum((x - u) ** 2 for x in f)), (K-1 or 1.0))
    o = sqrt(o)
    return tuple([old_div(v, (K or 1.0)) for v in m] + [o])
...
class NB(Classifier):
    ...
    def classify(self, document, discrete=True):
        ...
        for type in self._classes:
            ...
            g = sum(log(old_div((L[f] if f in L else a), d)) for f in v)
...
class BPNN(Classifier):
    ...
    def classify(self, document, discrete=True):
        ...
        if not self._trained:
            ...
            H2 = dict((x, old_div(i,n)) for i, x in enumerate(self.classes))  # Class => float hash (0.0-1.0).
            H3 = dict((old_div(i,n), x) for i, x in enumerate(self.classes))  # Class reversed hash.
...
class GeneticAlgorithm(object):
    ...
    def avg(self):
        ...
        return old_div(float(sum(map(self.fitness, self.population))), len(self.population))
