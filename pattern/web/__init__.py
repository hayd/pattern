...
class URL(object):
    ...
    def _parse(self):
        ...
        """
        p = urllib_parse.urlsplit(self._string)
        P = {PROTOCOL: p[0],            # http
             USERNAME: u"",             # user
             PASSWORD: u"",             # pass
    ...
    def open(self, timeout=10, proxy=None, user_agent=USER_AGENT, referrer=REFERRER, authentication=None):
        ...
        handlers.append(urllib.request.HTTPCookieProcessor(http_cookiejar.CookieJar()))
...
def base(url):
    ...
    """
    return urllib_parse.urlparse(url).netloc
...
def abs(url, base=None):
    ...
    if url.startswith("#") and not base is None and not base.endswith("/"):
        if not re.search("[^/]/[^/]", base):
            base += "/"
    return urllib_parse.urljoin(base, url)
