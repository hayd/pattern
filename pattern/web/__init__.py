...
def download(url=u"", method=GET, query={}, timeout=10, cached=True, throttle=0, proxy=None, user_agent=USER_AGENT, referrer=REFERRER, authentication=None, str=False):
    """ Downloads the content at the given URL (by default it will be cached locally).
        Unless unicode=False, the content is returned as a unicode string.
    """
...
class Google(SearchEngine):
    ...
    def search(self, query, type=SEARCH, start=1, count=10, sort=RELEVANCY, size=None, cached=True, **kwargs):
        ...
        if not query or count < 1 or start < 1 or start > (old_div(100, count)):
...
class Yahoo(SearchEngine):
    ...
    def search(self, query, type=SEARCH, start=1, count=10, sort=RELEVANCY, size=None, cached=True, **kwargs):
        ...
        if type == NEWS:
            url = YAHOO + "news"
        if not query or count < 1 or start < 1 or start > old_div(1000, count):
...
class Twitter(SearchEngine):
    ...
    def search(self, query, type=SEARCH, start=1, count=10, sort=RELEVANCY, size=None, cached=False, **kwargs):
        ...
        if type != SEARCH:
            raise SearchEngineTypeError
        if not query or count < 1 or (isinstance(start, (int, int, float)) and start < 1):
            return Results(TWITTER, query, type)
        if not isinstance(start, (int, int, float)):
        ...
        if isinstance(start, (int, int, float)):
...
class Flickr(SearchEngine):
    ...
    def search(self, query, type=IMAGE, start=1, count=10, sort=RELEVANCY, size=None, cached=True, **kwargs):
        ...
        if type not in (SEARCH, IMAGE):
            raise SearchEngineTypeError
        if not query or count < 1 or start < 1 or start > old_div(500,count):
...
def sort(terms=[], context="", service=GOOGLE, license=None, strict=True, prefix=False, **kwargs):
    ...
    R = [(old_div((r.total or 1),s), r.query) for r in R]
...
class Selector(object):
    ...
    def search(self, e):
        ...
        if len(self.classes) == 0 or len(self.classes) >= 2:
            e = list(map(Element, e._p.findAll(tag, **a)))
        if len(self.classes) == 1:
            e = list(map(Element, e._p.findAll(tag, **dict(a, **{"class": i(list(self.classes)[0])}))))
...
class Crawler(object):
    ...
    def push(self, link, priority=1.0, sort=FILO):
        ...
        if not isinstance(link, Link):
            link = Link(url=link)
        dt = time.time()
        dt = sort == FIFO and dt or old_div(1, dt)
